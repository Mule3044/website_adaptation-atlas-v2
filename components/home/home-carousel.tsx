...
const HomeCarousel = () => {
  ...
  const publishedSpotlights = spotlights?.filter((spotlight: Spotlight) => !spotlight.comingSoon)

  if (homeContent && spotlights) return (
        ...
        <CarouselContent className='h-[600px] lg:h-[800px]'>
          {publishedSpotlights.map((spotlight: Spotlight) =>
            <CarouselItem key={spotlight._id} className='relative flex justify-center items-center'>
              ...
              <h3 className='uppercase text-grey-600 font-medium mb-0.5'>{homeContent.spotlightCarousel?.kicker}</h3>
                {spotlight.carousel?.title && <h2 className='carousel-header'>{spotlight.carousel?.title}</h2>}
                ...
                <Button variant={'link'} size={'lg'} asChild className='group'>
                  <div>
                    <Link href={`/data-spotlights/${spotlight.slug}`}>{homeContent.spotlightCarousel?.buttonLabel}</Link>
